...
# timer.run()
pn_map, seed = generate_perlin_map(shape=(32, 32), scale=12, octaves=1, seed=1)
# timer.stop("Perlin map sin normalizar generado")
plt.plot_map(pn_map, min=-1, max=1, title=f"Perlin Map without normalization, seed={seed}")
...
# plt.plot_map(pn_map_norm, min=-1, max=1, title=f"Perlin Map, normalized, same seed={seed}")

# variating scale
# pn_maps_scales = [generate_perlin_map(scale=s, seed=seeds[0])[0] for s in scales]
# plt.plot_maps(pn_maps_scales, ('scale', scales), min=-1, max=1)
...
# plt.plot_maps(pn_maps_octaves, ('octaves', octaves), min=-1, max=1, title="Maps variating octaves, with same seed, non-normalized")
# pn_maps_octaves = [generate_perlin_map(scale=400, octaves=o, seed=seeds[0], normalized=True)[0] for o in octaves]
# plt.plot_maps(pn_maps_octaves, ('octaves', octaves), min=-1, max=1, title="Maps variating octaves, with same seed, normalized")
...
# plt.plot_maps(pn_maps_persistence, ('persistence', persistences), min=-1, max=1)
...
# plt.plot_maps(pn_maps_lacunarity, ('lacunarity', lacunarities), min=-1, max=1)

# variating seed
# pn_maps_seed = [generate_perlin_map(scale=400, octaves=3, seed=s, normalized=True)[0] for s in seeds]
# plt.plot_maps(pn_maps_seed, ('seed', seeds), min=-1, max=1)
...
# timer.run()
ds_map, seed, initial_corners = (ds(n=8, roughness=1))
# timer.stop("Diamond-square map generado")
corners_str = ",".join([str(c) for c in initial_corners])
plt.plot_map(ds_map, title=f"Map with seed {seed} and corners {corners_str}")

# variating n
# ds_maps_n = [ds(n, seed=ss[0], initial_corners=cs)[0] for n in ns]
# plt.plot_maps(ds_maps_n, ('N', ns), title="Maps variating N, with same seed and corners")

# variating roughhness
# ds_maps_rg = [ds(7, roughness=r, seed=ss[0], initial_corners=cs)[0] for r in rs]
# plt.plot_maps(ds_maps_rg, ('roughness', rs), title="Maps variating roughness, with same seed and corners") 

# variating seed
# ds_maps_sd = [ds(6, roughness=1, seed=s, initial_corners=cs)[0] for s in ss]
# plt.plot_maps(ds_maps_sd, ('seed', ss), title="Maps variating seed")
...
# plt.plot_maps(ds_maps_cr, ('seed/corners', [(seed1, corners1), (seed1, corners1), (seed1, None), (None, corners1)]), title="Maps variating seed and corners")
...
# plt.plot_maps(ds_maps_wrap, ('wrap/corners', [(True, cs), (False, cs), (True, cs_2), (False, cs_2)]), title="Maps with wrap vs. no-wrap")